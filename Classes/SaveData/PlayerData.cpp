...
{
	return &m_latest.m_HpData.at(id%4);
...
void PlayerData::levelUp()
{
	...
	if (m_level > 5)
	{
		m_latest.m_HpData[m_level % 4] = *Ruler::getInstance()->multiplay(&m_latest.m_HpData[m_level % 4], 9.54);
		if (m_level % 5 == 0)
		{
...
void PlayerData::heroLevelUp()
{
	if (m_playerLevel < 9)
	{
		m_basedps = SqLite::getInstance()->getDps(m_playerLevel - 1);
		m_playerLevel++;
	}
	else
	{
